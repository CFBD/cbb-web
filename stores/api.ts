...
export const useApiStore = defineStore("api", () => {
      ...
      if (currentRoute) {
        for (let key of Object.keys(queryParams.value)) {
          if (queryParams.value[key]) {
            if (key.indexOf("Date") !== -1 || key === 'date') {
              // @ts-ignore
              currentRoute.query[key] = (queryParams.value[key]).toISOString() as LocationQueryValue;
  ...
  function query() {
    if (selectedEndpoint.value) {
      ...
      let requestRoute = selectedEndpoint.value.key;
      for (let key of Object.keys(queryParams.value)) {
        requestRoute = requestRoute.replace(`{${key}}`, queryParams.value[key]?.toString() ?? '');
      }

      $fetch<any>(requestRoute, {
        method: "GET",
        baseURL: config.public.apiBaseUrl,
